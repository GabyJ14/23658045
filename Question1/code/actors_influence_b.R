#actors influence baby names: boys

actors_influence_b <- function(national_df, HBO_credits, HBO_titles) {
    ...
    b_influencal_names_shows <- national_df %>%
        filter(Name %in% HBO_top_90_actors_shows_names$First_Name) %>%
        filter(Year %in% (1990:2010)) %>%
        filter(Gender == "M") %>%
        filter(Total_Count_year > 500)
    ...
    plot_b <- ggplot(b_influencal_names_shows, aes(x = Year, y = Total_Count_year, color = Name)) +
        geom_line(size = 1) +  # Line plot
        geom_point(size = 2) +  # Add points on the line
        labs(
            title = "Influence of Top Actors' Names on Baby Boy Naming Trends",
            x = "Year",
            y = "Total Count",
            color = "Name"
        ) +
